...
public class EFCoreTranscriptStore: ITranscriptStore
{
    ...
    public EFCoreTranscriptStore(string connectionString)
    ...
    public EFCoreTranscriptStore(TranscriptStoreOptions options)
    {
        if (options == null)
        {
...
#pragma warning restore CS8619 // Nullability of reference types in value doesn't match target type.
    ...
    /// <returns>A <see cref="Task"/> A task that represents the work queued to execute.</returns>
    public Task<PagedResult<TranscriptInfo>> ListTranscriptsAsync(string channelId, string? continuationToken = default)
    {
        if (string.IsNullOrEmpty(channelId))
        {
            throw new ArgumentNullException($"missing {nameof(channelId)}");
